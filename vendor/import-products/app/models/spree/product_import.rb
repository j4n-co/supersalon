...
class  Spree::ProductImport < ActiveRecord::Base
  ...
  def create_product_using(params_hash)
    ...
    #working save
    #@new = Spree::Product.create({sku:'test5', name:'testprod3', description:'um',shipping_category_id:1, price:24.83})
    
    params_hash[:shipping_category_id] = Spree::ShippingCategory.where(name: params_hash[:shipping_category_name]).first.try(:id) || Spree::ShippingCategory.first.id
    params_hash.delete(:shipping_category_name)
    params_hash[:tax_category_id] = Spree::TaxCategory.where(name: params_hash[:tax_category_name]).first.try(:id) || Spree::TaxCategory.first.id
    params_hash.delete(:tax_category_name)
      
    price_in_pln = params_hash[:pln].to_f
    params_hash.delete(:pln)
    price_in_eur = params_hash[:eur].to_f
    params_hash.delete(:eur)
    ...
    product.save!
    # saving prices
    prodVariant = Spree::Variant.where(product_id: product.id).first
    
    euro = Spree::Price.new({:currency=>'EUR', :amount=>price_in_eur, :variant_id => prodVariant.id })
    euro.save!
    
    pln = Spree::Price.new({:currency=>'PLN', :amount=>price_in_pln, :variant_id => prodVariant.id })
    pln.save!
    ...
    #Spree::ShippingCategory.find(name: params_hash[:shipping_category_name]).first.id Spree::ShippingCategory.first.id
    ...
    if @names_of_products_before_import.include? product.name
      ...
      associate_product_with_properties(product, params_hash[:properties])
      
      unless params_hash[:count_on_hand].nil?
        add_stock_levels_to_product(product, params_hash[:count_on_hand], params_hash[:backorderable])
      end
  ...
  def associate_product_with_properties(product, properties)
    
    properties = properties.split(',')
    properties.map!{|p| p.split(':') }
    
    properties.each do |prop|
      product.set_property(prop[0], prop[1])
    end
    
  end

  def add_stock_levels_to_product(product, count_on_hand, backorderable)
    variant = Spree::Variant.where(:product_id => product.id).first
    stock = Spree::StockItem.where(:variant_id => variant.id).first
    backorderable = ActiveRecord::ConnectionAdapters::Column.value_to_boolean(backorderable)
    stock.backorderable = backorderable
    stock.save
    stock.adjust_count_on_hand(count_on_hand.to_i)
  end
  ...
  def associate_product_with_taxon(product, taxonomy, taxon_hierarchy)
    ...
    taxon_hierarchy.split(/\s*\&\s*/).each do |hierarchy|
      ...
      hierarchy.each do |taxon|
        current_taxon = Spree::Taxon.find_or_create_by_name_and_taxonomy_id(taxon, taxonomy.id)
        current_taxon.parent_id = root_taxon.id
        current_taxon.save!
        product.taxons << current_taxon unless product.taxons.include?(current_taxon)

      end
      taxonomy.save!
      #product.taxons << taxonomy.root unless product.taxons.include?(taxonomy.root)
